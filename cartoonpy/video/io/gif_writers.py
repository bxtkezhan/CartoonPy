...
from cartoonpy.decorators import (requires_duration,use_clip_fps_by_default)
...
def write_gif_with_tempfiles(clip, filename, fps=None, program= 'ImageMagick',
    ...
    if program == "ImageMagick":
        verbose_print(verbose, "[CartoonPy] Optimizing GIF with ImageMagick... ")
...
def write_gif(clip, filename, fps=None, program= 'ImageMagick',
    ...
    try:
        ...
        raise IOError(error)
    if program == 'ImageMagick':
        verbose_print(verbose, "[CartoonPy] Optimizing the GIF with ImageMagick...\n")
    proc1.stdin.close()
    proc1.wait()
    if program == 'ImageMagick':
        proc2.wait()
