...
import numpy as np
from cartoonpy.config import get_setting  # ffmpeg, ffmpeg.exe, etc...
    ...
    def read_frame(self):
        ...
        if len(s) != nbytes:
            ...
            if not hasattr(self, 'lastread'):
                raise IOError(("CartoonPy error: failed to read the first frame of "
...
def ffmpeg_read_image(filename, with_mask=True):
    ...
    This function is not meant to be used directly in CartoonPy,
...
def ffmpeg_parse_infos(filename, print_infos=False, check_duration=True):
    ...
    if check_duration:
        try:
            ...
            raise IOError(("CartoonPy error: failed to read the duration of file %s.\n"
    ...
    if result['video_found']:
        ...
        try:
            ...
            raise IOError(("CartoonPy error: failed to read video dimensions in file %s.\n"
                           "Here are the file infos returned by ffmpeg:\n\n%s")%(
                              filename, infos))
